...
{
	tegra_machine_restart(mode, cmd);
}
...
{
	char *cmd = data;
	unsigned int *tag = IO_ADDRESS(TEGRA_IRAM_BASE +
						TEGRA_IRAM_SIZE -
						sizeof(unsigned int));

	/*tag for reboot into fastboot download mode */
	if (cmd && !strcmp(cmd, "bootloader")) {
		*tag = REBOOT_TO_BOOTLOADER;
	} else if (cmd && !strcmp(cmd, "recovery")) {
		*tag = REBOOT_TO_RECOVERY;
	} else {
	/*clear the flag after recovery mode process completed.*/
		*tag = 0;
	}
...
void __init tegra_init_reboot(void)
{
	register_reboot_notifier(&tegra_reboot_nb);
}
