...
static int ulpi_phy_power_on(struct tegra_usb_phy *phy, bool is_dpd)
{
	...
	if (coldboot) {
		...
		tegra_gpio_enable(config->reset_gpio);
		gpio_request(config->reset_gpio, "ulpi_phy_reset_b");
		gpio_export(config->reset_gpio,1);
			...
			enum tegra_usb_phy_type usb_phy_type)
{
	...
	if (phy_is_ulpi(phy)) {
		...
		if (ulpi_config->inf_type == TEGRA_USB_LINK_ULPI) {
			...
			gpio_export(ulpi_config->reset_gpio,1);
			gpio_direction_output(ulpi_config->reset_gpio, 0);
			gpio_free(ulpi_config->reset_gpio);
