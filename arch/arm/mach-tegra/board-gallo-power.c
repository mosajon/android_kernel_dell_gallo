...
static struct board_version b_version_power;

int __init gallo_charge_init(void)
{
...
static struct regulator_consumer_supply tps658621_ldo0_supply[] = {
	...
	REGULATOR_SUPPLY("vdd_vibrator", NULL),
};
...
static struct i2c_board_info __initdata gallo_regulators[] = {
	{
...
static struct tegra_suspend_platform_data gallo_suspend_data = {
	/*
...
int __init gallo_regulator_init(void)
{
	...
	if (minor == 3 && !(tegra_spare_fuse(18) || tegra_spare_fuse(19)))
		gallo_suspend_data.suspend_mode = TEGRA_SUSPEND_LP1;

	/* configure the power management controller to trigger PMU
	 * interrupts when low */
	pmc_ctrl = readl(pmc + PMC_CTRL);
	writel(pmc_ctrl | PMC_CTRL_INTR_LOW, pmc + PMC_CTRL);
	i2c_register_board_info(4, gallo_regulators, 1);
	gallo_get_hw_info(&b_version_power);
	if (b_version_power.hw_version == BOARD_VERSION_EVT1) {
		gallo_suspend_data.wake_enb |= (TEGRA_WAKE_GPIO_PQ7 | TEGRA_WAKE_GPIO_PQ6);
		gallo_suspend_data.wake_any |= (TEGRA_WAKE_GPIO_PQ7 | TEGRA_WAKE_GPIO_PQ6);
	}

	regulator_has_full_constraints();

	tegra_init_suspend(&gallo_suspend_data);
	return 0;
}

static int __init gallo_pcie_init()
{
	int ret;
