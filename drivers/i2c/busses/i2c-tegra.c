...
#define TEGRA_I2C_TIMEOUT			(msecs_to_jiffies(500))
...
{
	u32 status, sl_status;
	const u32 status_err = I2C_INT_NO_ACK | I2C_INT_ARBITRATION_LOST | I2C_INT_TX_FIFO_OVERFLOW;
	struct tegra_i2c_dev *i2c_dev = dev_id;

	if (i2c_dev->is_slave) {
		sl_status = i2c_readl(i2c_dev, I2C_I2C_SL_STATUS);
		if (sl_status & I2C_I2C_SL_STATUS_SL_IRQ) {
			dev_warn(i2c_dev->dev, "slave interrupt: status: 0x%x\n", sl_status);
 ...
 * i2c protocol Rev. 03 section 3.16 titled "Bus clear"
 ...
 return:
	0	: The bus is no error
	EALREADY: The bus error has been recovered
	-EIO	: The bus error and can't be recovered
 */
static int i2c_recover_bus(struct tegra_i2c_bus *i2c_bus, u16 client_addr)
{
	u32 pulse_count = 9;
	int res = -EIO;
	int i, scl_state, sda_state;
	struct tegra_i2c_dev *i2c_dev = i2c_bus->dev;
	struct tegra_i2c_platform_data *plat = i2c_dev->dev->platform_data;

	/*only recover GEN1 currently*/
	if(i2c_bus->adapter.nr != 0)
		return res;

	dev_info(i2c_dev->dev, "initiating i2c bus recovery\n");

	tegra_gpio_enable(TEGRA_GPIO_PC4);
	gpio_request(TEGRA_GPIO_PC4, "gen1_scl_gpio");
	gpio_direction_input(TEGRA_GPIO_PC4);
	scl_state = gpio_get_value(TEGRA_GPIO_PC4);

	tegra_gpio_enable(TEGRA_GPIO_PC5);
	gpio_request(TEGRA_GPIO_PC5, "gen1_sda_gpio");
	gpio_direction_input(TEGRA_GPIO_PC5);
	sda_state = gpio_get_value(TEGRA_GPIO_PC5);

	dev_info(i2c_dev->dev, "i2c clock=%d, data=%d\n", scl_state, sda_state);
	...
	/*SCL is H, SDA is L, try to recover SDA*/
	if(scl_state && !sda_state) {
		/*try to send out the NACK and STOP*/
		/*generate 9 clk pulse*/
		for (i = 0; i < pulse_count; i++) {
			...
			if(gpio_get_value(TEGRA_GPIO_PC5)) {
				/*both SCL and SDA are H now*/

				/*send START: SDA:H->L (SCL:H)*/
				...
				/*send STOP: SDA:L->H (SCL:H)*/
				gpio_direction_output(TEGRA_GPIO_PC5, 1);
				udelay(5);

				pr_err("I2C bus be recoverred successful\n");
	...
	pr_err("I2C bus cannot be recoverred, try to reset devices...\n");

	if(plat->i2c_bus_reset_devices) {
		(plat->i2c_bus_reset_devices)(plat, client_addr);
	}
	...
	dev_info(i2c_dev->dev, "i2c bus recovery complete!\n");
	...
	struct i2c_msg *msg, int stop)
{
	...
	if (WARN_ON(ret == 0)) {
		/*timeout*/
		dev_err(i2c_dev->dev,
			"i2c transfer timed out, addr 0x%04x, data 0x%02x, err:%d\n",
			msg->addr, msg->buf[0], i2c_dev->msg_err);
	} else {
		/*complete*/
	...
	if (i2c_dev->msg_err == I2C_ERR_NO_ACK) {
		...
		/*recover bus and reinit*/
		recover_res = i2c_recover_bus(i2c_bus, msg->addr);
		tegra_i2c_init(i2c_dev);
	}

	/*retry at the bus not be recovered*/
	if(!recover_res)
		return -EAGAIN;
	else
		return -EIO;
