...
#define MPU3050_EARLY_SUSPEND_IN_DRIVER 0
	...
	/*get the calibration data from EEPROM begin*/
#define ML_INIT_CAL_LEN          (EEPROM_SIZE_GSENSOR+EEPROM_SIZE_GYRO)          /* length in bytes of calibration data file      */

	/*get the calibration data from EEPROM end*/
...
int MLSLReadCal_flash(unsigned char *data)
{
	tMLError result = ML_ERROR;
	printk(KERN_DEBUG "%s() ++\n", __func__);

	struct i2c_adapter *eeprom_adapter;
	eeprom_adapter = i2c_get_adapter(4);

	if (eeprom_adapter == NULL || data == NULL) {
		printk(KERN_ERR "%s Can not get i2c adapter 4, %x,%x  \n", __func__, eeprom_adapter, data);
		return result;
	}

	result = MLSLSerialRead(eeprom_adapter, EEPROM_SLAVE_ADDRESS, EEPROM_OFFSET_GSENSOR, ML_INIT_CAL_LEN, data);

	if (result != ML_SUCCESS) {
		printk(KERN_ERR "%s() Read Calibration data from flash Error \n", __func__);
		return result;
		      ...
		      unsigned int cmd, unsigned long arg)
{
	...
	case MPU_I2C_R_CDATA:
		{
		unsigned char data[ML_INIT_CAL_LEN];
		retval = MLSLReadCal_flash(data);
		  ...
		  const struct i2c_device_id *devid)
{
	...
	if (res < 0) {
		dev_err(&this_client->adapter->dev,
			"ERROR: register_pm_notifier returned %d\n", res);
