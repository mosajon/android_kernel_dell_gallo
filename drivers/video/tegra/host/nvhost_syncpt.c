...
void nvhost_syncpt_save(struct nvhost_syncpt *sp)
{
	...
	for (i = 0; i < NV_HOST1X_SYNCPT_NB_PTS; i++) {
		...
		{
			bool ret = nvhost_syncpt_min_eq_max(sp, i);
			if( !ret )
				nvhost_syncpt_update_min(sp, i);
...
{
	struct nvhost_master *dev = syncpt_to_dev(sp);
	wait_event_timeout(dev->mod.idle, dev->mod.powered,
			   ACM_TIMEOUT + msecs_to_jiffies(1500));
	WARN_ON(!nvhost_module_powered(&dev->mod));
	WARN_ON(!client_managed(id) && nvhost_syncpt_min_eq_max(sp, id));
			...
			u32 thresh, u32 timeout, u32 *value)
{
	...
	if (id >= NVSYNCPT_VI_ISP_0 && id <= NVSYNCPT_VI_ISP_5)
		timeout = 1500;
	while (timeout) {
