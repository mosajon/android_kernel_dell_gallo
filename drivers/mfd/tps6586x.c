...
#define TPS6586X_MAX_I2C_RETRY_COUNTS 20
...
static inline int __tps6586x_read_retry(struct i2c_client *client,
				  int reg, uint8_t *val)
{
	int i, ret;
	for(i=0; i < TPS6586X_MAX_I2C_RETRY_COUNTS; i++) {
		ret = __tps6586x_read(client, reg, val);
		if (ret >= 0) {
			return ret;
		}
...
static inline int __tps6586x_reads_retry(struct i2c_client *client, int reg,
				   int len, uint8_t *val)
{
	int i, ret;
	for(i=0; i < TPS6586X_MAX_I2C_RETRY_COUNTS; i++) {
		ret = __tps6586x_reads(client, reg, len, val);
		if (ret >= 0) {
			return ret;
		}
		dev_err(&client->dev, "%s:retry:%d\n", __FUNCTION__, i);
...
static inline int __tps6586x_write_retry(struct i2c_client *client,
				 int reg, uint8_t val)
{
	int i, ret;
	for(i=0; i < TPS6586X_MAX_I2C_RETRY_COUNTS; i++) {
		ret = __tps6586x_write(client, reg, val);
		if (ret >= 0) {
			return ret;
		}
		dev_err(&client->dev, "%s:retry:%d\n", __FUNCTION__, i);
...
static inline int __tps6586x_writes_retry(struct i2c_client *client, int reg,
				  int len, uint8_t *val)
{
	int ret, i;

	for (i = 0; i < len; i++) {
		ret = __tps6586x_write_retry(client, reg + i, *(val + i));
		if (ret < 0)
			return ret;
	}
...
{
	return __tps6586x_write_retry(to_i2c_client(dev), reg, val);
}
EXPORT_SYMBOL_GPL(tps6586x_write);

int tps6586x_writes(struct device *dev, int reg, int len, uint8_t *val)
{
	return __tps6586x_writes_retry(to_i2c_client(dev), reg, len, val);
}
EXPORT_SYMBOL_GPL(tps6586x_writes);

int tps6586x_read(struct device *dev, int reg, uint8_t *val)
{
	return __tps6586x_read_retry(to_i2c_client(dev), reg, val);
}
EXPORT_SYMBOL_GPL(tps6586x_read);

int tps6586x_reads(struct device *dev, int reg, int len, uint8_t *val)
{
	return __tps6586x_reads_retry(to_i2c_client(dev), reg, len, val);
}
...
int tps6586x_set_bits(struct device *dev, int reg, uint8_t bit_mask)
{
	...
	ret = __tps6586x_read_retry(to_i2c_client(dev), reg, &reg_val);
	if (ret)
		goto out;

	if ((reg_val & bit_mask) == 0) {
		reg_val |= bit_mask;
		ret = __tps6586x_write_retry(to_i2c_client(dev), reg, reg_val);
	}
out:
...
int tps6586x_update(struct device *dev, int reg, uint8_t val, uint8_t mask)
{
	...
	ret = __tps6586x_read_retry(tps6586x->client, reg, &reg_val);
	if (ret)
		goto out;

	if ((reg_val & mask) != val) {
		reg_val = (reg_val & ~mask) | val;
		ret = __tps6586x_write_retry(tps6586x->client, reg, reg_val);
	}
out:
	mutex_unlock(&tps6586x->lock);
	return ret;
...
static int tps6586x_gpio_get(struct gpio_chip *gc, unsigned offset)
{
	...
	ret = __tps6586x_read_retry(tps6586x->client, TPS6586X_GPIOSET2, &val);
	if (ret)
		return ret;
			      ...
			      int value)
{
	...
	__tps6586x_write_retry(tps6586x->client, TPS6586X_GPIOSET2,
			 value << offset);
}
...
static irqreturn_t tps6586x_irq(int irq, void *data)
{
	...
	if (ret < 0) {
		dev_err(tps6586x->dev, "failed to read interrupt status:irq:%d\n",irq);
		return IRQ_NONE;
	}
