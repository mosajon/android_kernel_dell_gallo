...
static int tps6586x_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	struct tps6586x_rtc *rtc = dev_get_drvdata(dev);
	struct device *tps_dev = to_tps6586x_dev(dev);
	unsigned long long ticks, ticks_read = 0;
	unsigned long seconds, seconds_read;
	u8 buff[5], buff_read[5];
	int err;
	int i, loop = 10;
	...
	/* temporary soluction for seting RTC fail */
	do {
		...
		err = tps6586x_reads(tps_dev, RTC_COUNT4,
					sizeof(buff_read), buff_read);
		if (err < 0) {
			dev_err(dev, "failed to read counter\n");
			return err;
		}

		for (i = 0; i < sizeof(buff_read); i++) {
			ticks_read <<= 8;
			ticks_read |= buff_read[i];
		}
		seconds_read = ticks_read >> 10;

		if (seconds == seconds_read)
			loop = 0;
		else
			loop--;
...
static int tps6586x_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	struct device *tps_dev = to_tps6586x_dev(dev);
	unsigned long seconds, seconds_read;
	unsigned long ticks, ticks_read = 0;
	u8 buff[3], buff_read[3];
	int err;
	int loop = 10;
	...
	/* temporary soluction for seting alarm fail */
	do {
		err = tps6586x_writes(tps_dev, RTC_ALARM1_HI,
					sizeof(buff), buff);
		if (err) {
		...
		err = tps6586x_reads(tps_dev, RTC_ALARM1_HI,
					sizeof(buff_read), buff_read);
		if (err)
			return err;

		ticks_read = (buff_read[0] << 16) | (buff_read[1] << 8) |
					buff_read[2];
		seconds_read = ticks_read >> 10;
