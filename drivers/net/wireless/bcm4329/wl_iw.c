...
#include <wl_iw.h>
extern struct mutex stop_mmc_rescan;
...
extern int DHD_suspend_resume_FLAG;
...
#ifndef CUSTOMER_HW2
			dev_wlc_ioctl(dev, WLC_SET_PM, &pm_local, sizeof(pm_local));
			WL_ERROR(("%s: power mode set %d\n", __FUNCTION__, pm_local));
#endif
...
)
{
	...
	if (DHD_suspend_resume_FLAG) {
		iw = *(wl_iw_t **) netdev_priv(dev);
		/* Kernel resumed  */
		WL_ERROR(("%s: Remove extra suspend setting \n", __FUNCTION__));

		dhdcdc_set_ioctl(iw->pub, 0, WLC_SET_PM, (char *)&power_mode, sizeof(power_mode));

		/* disable pkt filter */
		dhd_set_packet_filter(0, iw->pub);

		/* restore pre-suspend setting for dtim_skip */
		bcm_mkiovar("bcn_li_dtim", (char *)&(iw->pub)->dtim_skip, 4, iovbuf, sizeof(iovbuf));

		dhdcdc_set_ioctl(iw->pub, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
#ifdef CUSTOMER_HW2
		bcm_mkiovar("roam_off", (char *)&roamvar, 4, iovbuf, sizeof(iovbuf));
		dhdcdc_set_ioctl(iw->pub, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
#endif /* CUSTOMER_HW2 */
		DHD_suspend_resume_FLAG = 0;
	}
	net_os_wake_lock(dev);
...
)
{
	int ret = 0;
	wl_iw_t *iw;
	if (mutex_is_locked(&stop_mmc_rescan))
		printk(KERN_ERR "stop_mmc_rescan is locked \n");
...
)
{
	...
	if ((ret = wl_control_wl_start(dev)) != BCME_OK) {
		...
		if ((ret = wl_control_wl_start(dev)) != BCME_OK) {
			...
			printk(KERN_ERR "stop_mmc_rescan is unlocked \n");
