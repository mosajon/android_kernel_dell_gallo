				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->bias_level == SND_SOC_BIAS_OFF) {
			...
			snd_soc_write(codec, WM8903_WRITE_SEQUENCER_0, 0x111);
...
{
	unsigned int val = connect ? 1 : 0;
	snd_soc_dapm_update_mixer(codec, "DACR", "Right Output Mixer",
		"DACR Switch", val);
	snd_soc_dapm_update_mixer(codec, "DACL", "Left Output Mixer",
		"DACL Switch", val);

	return 0;
}
EXPORT_SYMBOL_GPL(wm8903_connect_dac_to_mix);

int wm8903_connect_dac_to_mixspk(struct snd_soc_codec *codec, bool connect)
{
	unsigned int val = connect ? 1 : 0;
	snd_soc_dapm_update_mixer(codec, "DACR", "Right Speaker Mixer",
		"DACR Switch", val);
	snd_soc_dapm_update_mixer(codec, "DACL", "Left Speaker Mixer",
		"DACL Switch", val);

	return 0;
}
EXPORT_SYMBOL_GPL(wm8903_connect_dac_to_mixspk);
	...
	bool int_mic, bool connect)
{
	...
	aif0 = snd_soc_read(codec, WM8903_AUDIO_INTERFACE_0);

	if (connect) {
		if (int_mic) {
			if (pdata->int_mic_l && pdata->int_mic_r) {
				aif0  |= (0x1 << WM8903_AIFADCR_SRC_SHIFT);
				aif0  &= ~(0x1 << WM8903_AIFADCL_SRC_SHIFT);
			} else if (!pdata->int_mic_l && pdata->int_mic_r) {
				// replicate mic right on both channels
				aif0  |= (0x1 << WM8903_AIFADCR_SRC_SHIFT);
				aif0  |= (0x1 << WM8903_AIFADCL_SRC_SHIFT);
			} else if (pdata->int_mic_l && !pdata->int_mic_r) {
				// replicate mic left on both channels
				aif0  &= ~(0x1 << WM8903_AIFADCR_SRC_SHIFT);
				aif0  &= ~(0x1 << WM8903_AIFADCL_SRC_SHIFT);
			} else {
			}
		} else {
			if (pdata->hs_mic_l && pdata->hs_mic_r) {
			} else if (!pdata->hs_mic_l && pdata->hs_mic_r) {
				// replicate mic right on both channels
				aif0  |= (0x1 << WM8903_AIFADCR_SRC_SHIFT);
				aif0  |= (0x1 << WM8903_AIFADCL_SRC_SHIFT);
			} else if (pdata->hs_mic_l && !pdata->hs_mic_r) {
	...
	snd_soc_write(codec, WM8903_AUDIO_INTERFACE_0, aif0);

	return 0;
}
EXPORT_SYMBOL_GPL(wm8903_config_digital_audio_channel_source);
...
int wm8903_apply_audio_gain_spk(struct snd_soc_codec *codec)
{
	...
	dac_vol_left = snd_soc_read(codec, WM8903_DAC_DIGITAL_VOLUME_LEFT);
	dac_vol_right = snd_soc_read(codec, WM8903_DAC_DIGITAL_VOLUME_RIGHT);
	aout3_left = snd_soc_read(codec, WM8903_ANALOGUE_OUT3_LEFT);
	aout3_right = snd_soc_read(codec, WM8903_ANALOGUE_OUT3_RIGHT);

	dac_vol_left &= ~(WM8903_DACL_VOL_MASK);
	dac_vol_left |= pdata->dacl_vol;
	dac_vol_right &= ~(WM8903_DACR_VOL_MASK);
	dac_vol_right |= pdata->dacr_vol | WM8903_DACVU;
	aout3_left &= ~(WM8903_SPKL_VOL_MASK);
	aout3_left |= pdata->spkl_vol;
	aout3_right &= ~(WM8903_SPKR_VOL_MASK);
	aout3_right |= pdata->spkr_vol | WM8903_SPKVU;

	snd_soc_write(codec, WM8903_DAC_DIGITAL_VOLUME_LEFT, dac_vol_left);
	snd_soc_write(codec, WM8903_DAC_DIGITAL_VOLUME_RIGHT, dac_vol_right);
	snd_soc_write(codec, WM8903_ANALOGUE_OUT3_LEFT, aout3_left);
	snd_soc_write(codec, WM8903_ANALOGUE_OUT3_RIGHT, aout3_right);

	return 0;
}
EXPORT_SYMBOL_GPL(wm8903_apply_audio_gain_spk);
...
int wm8903_apply_audio_gain_headphone(struct snd_soc_codec *codec)
{
	...
	dac_vol_left = snd_soc_read(codec, WM8903_DAC_DIGITAL_VOLUME_LEFT);
	dac_vol_right = snd_soc_read(codec, WM8903_DAC_DIGITAL_VOLUME_RIGHT);
	aout1_left = snd_soc_read(codec, WM8903_ANALOGUE_OUT1_LEFT);
	aout1_right = snd_soc_read(codec, WM8903_ANALOGUE_OUT1_RIGHT);

	dac_vol_left &= ~(WM8903_DACL_VOL_MASK);
	dac_vol_left |= pdata->dacl_vol;
	dac_vol_right &= ~(WM8903_DACR_VOL_MASK);
	dac_vol_right |= pdata->dacr_vol | WM8903_DACVU;
	aout1_left &= ~(WM8903_HPOUTL_VOL_MASK);
	aout1_left |= pdata->hpoutl_vol;
	aout1_right &= ~(WM8903_HPOUTR_VOL_MASK);
	aout1_right |= pdata->hpoutr_vol | WM8903_HPOUTVU;

	snd_soc_write(codec, WM8903_DAC_DIGITAL_VOLUME_LEFT, dac_vol_left);
	snd_soc_write(codec, WM8903_DAC_DIGITAL_VOLUME_RIGHT, dac_vol_right);
	snd_soc_write(codec, WM8903_ANALOGUE_OUT1_LEFT, aout1_left);
	snd_soc_write(codec, WM8903_ANALOGUE_OUT1_RIGHT, aout1_right);

	return 0;
}
EXPORT_SYMBOL_GPL(wm8903_apply_audio_gain_headphone);
...
int wm8903_apply_audio_gain_lineout(struct snd_soc_codec *codec)
{
	...
	dac_vol_left = snd_soc_read(codec, WM8903_DAC_DIGITAL_VOLUME_LEFT);
	dac_vol_right = snd_soc_read(codec, WM8903_DAC_DIGITAL_VOLUME_RIGHT);
	aout2_left = snd_soc_read(codec, WM8903_ANALOGUE_OUT2_LEFT);
	aout2_right = snd_soc_read(codec, WM8903_ANALOGUE_OUT2_RIGHT);

	dac_vol_left &= ~(WM8903_DACL_VOL_MASK);
	dac_vol_left |= pdata->dacl_vol;
	dac_vol_right &= ~(WM8903_DACR_VOL_MASK);
	dac_vol_right |= pdata->dacr_vol | WM8903_DACVU;
	aout2_left &= ~(WM8903_LINEOUTL_VOL_MASK);
	aout2_left |= pdata->lineoutl_vol;
	aout2_right &= ~(WM8903_LINEOUTR_VOL_MASK);
	aout2_right |= pdata->lineoutr_vol | WM8903_LINEOUTVU;

	snd_soc_write(codec, WM8903_DAC_DIGITAL_VOLUME_LEFT, dac_vol_left);
	snd_soc_write(codec, WM8903_DAC_DIGITAL_VOLUME_RIGHT, dac_vol_right);
	snd_soc_write(codec, WM8903_ANALOGUE_OUT2_LEFT, aout2_left);
	snd_soc_write(codec, WM8903_ANALOGUE_OUT2_RIGHT, aout2_right);

	return 0;
}
EXPORT_SYMBOL_GPL(wm8903_apply_audio_gain_lineout);
...
int wm8903_apply_audio_gain_mic(struct snd_soc_codec *codec)
{
	...
	adc_vol_left = snd_soc_read(codec, WM8903_ADC_DIGITAL_VOLUME_LEFT);
	adc_vol_right = snd_soc_read(codec, WM8903_ADC_DIGITAL_VOLUME_RIGHT);

	adc_vol_left &= ~(WM8903_ADCL_VOL_MASK);
	adc_vol_left |= pdata->int_mic_adcl_vol;
	adc_vol_right &= ~(WM8903_ADCR_VOL_MASK);
	adc_vol_right |= pdata->int_mic_adcr_vol | WM8903_ADCVU;

	snd_soc_write(codec, WM8903_ADC_DIGITAL_VOLUME_LEFT, adc_vol_left);
	snd_soc_write(codec, WM8903_ADC_DIGITAL_VOLUME_RIGHT, adc_vol_right);

	return 0;
}
EXPORT_SYMBOL_GPL(wm8903_apply_audio_gain_mic);
...
int wm8903_apply_audio_gain_headset_mic(struct snd_soc_codec *codec)
{
	...
	adc_vol_left = snd_soc_read(codec, WM8903_ADC_DIGITAL_VOLUME_LEFT);
	adc_vol_right = snd_soc_read(codec, WM8903_ADC_DIGITAL_VOLUME_RIGHT);
	ain0_left = snd_soc_read(codec, WM8903_ANALOGUE_LEFT_INPUT_0);
	ain0_right = snd_soc_read(codec, WM8903_ANALOGUE_RIGHT_INPUT_0);

	adc_vol_left &= ~(WM8903_ADCL_VOL_MASK);
	adc_vol_left |= pdata->hs_mic_adcl_vol;
	adc_vol_right &= ~(WM8903_ADCR_VOL_MASK);
	adc_vol_right |= pdata->hs_mic_adcr_vol | WM8903_ADCVU;

	ain0_left &= ~(WM8903_LIN_VOL_MASK);
	ain0_left |= pdata->lin_vol;
	ain0_right &= ~(WM8903_RIN_VOL_MASK);
	ain0_right |= pdata->rin_vol;

	snd_soc_write(codec, WM8903_ADC_DIGITAL_VOLUME_LEFT, adc_vol_left);
	snd_soc_write(codec, WM8903_ADC_DIGITAL_VOLUME_RIGHT, adc_vol_right);
	snd_soc_write(codec, WM8903_ANALOGUE_LEFT_INPUT_0, ain0_left);
	snd_soc_write(codec, WM8903_ANALOGUE_RIGHT_INPUT_0, ain0_right);

	return 0;
}
EXPORT_SYMBOL_GPL(wm8903_apply_audio_gain_headset_mic);
