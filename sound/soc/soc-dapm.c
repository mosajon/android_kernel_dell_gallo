...
int snd_soc_dapm_update_mixer(struct snd_soc_codec *codec,
	const char *src_name, const char *dest_name, const char *control_name, unsigned int val)
{
	int max;
	unsigned int reg;
	unsigned int shift;
	...
	struct snd_soc_dapm_widget *widget = NULL;
	struct snd_soc_dapm_path *p;

	pr_debug("src_name %s, dest_name %s, control_name %s\n",
		src_name, dest_name, control_name);

	/* find widget */
	list_for_each_entry(p, &codec->dapm_paths, list) {
		pr_debug("path source name %s, path sink name %s\n",
			p->source->name, p->sink->name);
		if (!strcmp(p->source->name, src_name)) {
			if (!strcmp(p->sink->name, dest_name)) {
				pr_debug("widget found\n");
				widget = p->sink;
				break;
			}
		}
	...
	for (i = 0; i < widget->num_kcontrols; ++i) {
		if (!strcmp(control_name, widget->kcontrols[i].name)) {
			pr_debug("kcontrol found\n");
			kcontrol = p->kcontrol;
			break;
		}
	}
	if (!kcontrol) {
		pr_err("kcontrol not found\n");
		ret = 1;
		goto out;
	}
	...
	max =
		...
		(widget->kcontrols[i].private_value))->max;
	reg =
		((struct soc_mixer_control *)
		(widget->kcontrols[i].private_value))->reg;
	shift =
		((struct soc_mixer_control *)
		(widget->kcontrols[i].private_value))->shift;
	invert =
		((struct soc_mixer_control *)
		(widget->kcontrols[i].private_value))->invert;
	pr_debug("max %d, reg 0x%X, shift %d, invert %d\n",
		max, reg, shift, invert);
...
EXPORT_SYMBOL_GPL(snd_soc_dapm_update_mixer);
