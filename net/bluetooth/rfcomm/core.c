...
static int ua_disc_flag = RFCOMM_UADISC_FALSE;
	...
	if (dlci) {
		...
		case BT_DISCONN:
			...
			if (s->sock->sk->sk_state != BT_CLOSED) {
				ua_disc_flag = RFCOMM_UADISC_TRUE;
				rfcomm_session_put(s);
			}
			break;
		}
...
static inline void rfcomm_process_rx(struct rfcomm_session *s)
{
	...
	if ((sk->sk_state == BT_CLOSED) && (ua_disc_flag != RFCOMM_UADISC_TRUE)) {
		if (!s->initiator)
			rfcomm_session_put(s);

		rfcomm_session_close(s, sk->sk_err);
	}
	ua_disc_flag = RFCOMM_UADISC_FALSE;
}
